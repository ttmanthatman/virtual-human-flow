...
- 中间：房间时间线；默认合并当前角色下所有用户的消息，让同一个虚拟人面对多人聊天。当前用户仍写入自己的私有历史桶，但房间里能同时看到 Qoo、自己和虚拟人的对话。回复节奏为连续多条或短句爆发时，会把虚拟人的自然语言回复分成多条消息展示。每轮说话前会把场景、心理判断、记忆浮现和开口冲动作为“心理流”streaming 到聊天区；第一句说完后，前置心理流折叠成可展开卡片，发言后的状态余波也会 streaming 并在整轮结束后折叠。
- 右侧：流程追踪和生成监视，展示 `事件 -> 时空场景 -> 人物主脑 -> 评估 -> 记忆召回 -> 回应决策 -> 回应提示词 -> 回应输出 -> 状态更新 -> 信号评估 -> 状态变化`，也展示人物短预览、人物档案和场景生成的输入、流式输出和状态。
...
后台会记录每个登录用户的一次输入、虚拟人输出和本轮对话每个模块的调用记录，写入运行时文件 `.conversation-audits.local.json`；只有管理员可以在右侧“输入输出审计”里查看、选择导出、完整导出所有用户所有记录、删除单条或清空这些记录。所有登录用户默认看到当前人物的房间时间线，也可以在中间栏上方选择某个用户，只读查看该用户与该人物的私有历史消息；查看单个用户历史时不能发送，必须切回“房间时间线”后再继续对话或触发事件。`.conversation-histories.local.json`、`.conversation-states.local.json`、`.conversation-audits.local.json`、`.persona-dossiers.local.json` 和 `.deepseek.local.json` 都被 `.gitignore` 忽略，不能提交。
...
中间栏还提供非聊天的时间事件触发。用户输入一个时间后，系统会先把角色推进到该时间对应的生活现场，再调用事件活动 LLM，让人物在非聊天回合里产生心理活动、动作、位移、关系影响、记忆写入和可能的外显输出。活动生成会 streaming 到房间里，完成后折叠成可展开卡片。这个事件不是聊天消息，不要求角色一定说话，但会写入房间时间线和角色短期记忆，供下一轮任何用户或未来虚拟人自然承接。
...
流程追踪面板会在每个模块开始时自动切换到当前模块，并分区显示输入、输出和状态。外部 DeepSeek 调用使用流式返回，输出到达时会逐步更新；中间栏心理流来自同一条 pipeline progress，不改变“只有说出口段落进入聊天历史”的边界。人物短预览、人物档案生成和场景生成复用同一个监视窗口。
