...
流程追踪面板会在每个模块开始时自动切换到当前模块，并分区显示输入、输出和状态。外部 DeepSeek 调用使用流式返回，输出到达时会逐步更新；中间栏心理流来自同一条 pipeline progress，不改变 Reply LLM 只输出台词的边界。人物短预览、人物档案生成和场景生成复用同一个监视窗口。
