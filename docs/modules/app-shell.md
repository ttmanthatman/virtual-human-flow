...
- 输出：UI 状态、中间栏消息（含临时心理流、`roleTurn` 台词连续回复分段）、中间栏当前人物全局 `scene/location/runtime` 摘要、当前角色下各用户历史只读展示、右侧 `pipelineTrace`/`generationMonitor`、后台保存请求、管理员审计 JSON 下载、角色重置请求、更新窗口“本次更新”说明。
