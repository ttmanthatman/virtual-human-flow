...
App Shell 负责三栏工作台、前端交互状态、聊天入口、连续回复分条呈现、流程追踪、生成监视、档案切换、当前角色重置入口、管理员审计查看/选择/导出和浏览器端权限提示。它不负责服务端持久化规则、真实 DeepSeek 代理实现或认知模块内部判断。
...
- 输出：UI 状态、中间栏消息（含临时心理流、Reply LLM 连续回复分段）、中间栏当前人物全局 `scene/location/runtime` 摘要、右侧 `pipelineTrace`/`generationMonitor`、后台保存请求、管理员审计 JSON 下载、角色重置请求、更新窗口“本次更新”说明。
...
- 删除审计或重置当前角色后，前端必须同步清理当前消息桶、管理员历史桶和 localStorage，避免回主页面后看到旧记录。
