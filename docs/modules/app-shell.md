...
- 输出：UI 状态、中间栏消息（含临时心理流、Reply LLM 连续回复分段）、中间栏当前人物全局 `scene/location/runtime` 摘要、右侧 `pipelineTrace`/`generationMonitor`、后台保存请求、管理员审计 JSON 下载、更新窗口“本次更新”说明。
...
- `rg -n "liveTrace|activeStep|mindFlow|upsertMindFlowChatMessage|foldTransientMindFlowMessages|authUser|personaDossier|normalizeReplySegments|conversation-history|conversation-state|selectedAuditIds|exportConversationAuditEntries" src/App.tsx src/chat`
