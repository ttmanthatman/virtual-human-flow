...
App Shell 负责三栏工作台、前端交互状态、聊天入口、连续回复分条呈现、流程追踪、生成监视、档案切换、当前角色重置入口、共享角色历史选择器、管理员审计查看/选择/导出和浏览器端权限提示。它不负责服务端持久化规则、真实 DeepSeek 代理实现或认知模块内部判断。
...
- 输出：UI 状态、中间栏消息（含临时心理流、Reply LLM 连续回复分段）、中间栏当前人物全局 `scene/location/runtime` 摘要、当前角色下各用户历史只读展示、右侧 `pipelineTrace`/`generationMonitor`、后台保存请求、管理员审计 JSON 下载、角色重置请求、更新窗口“本次更新”说明。

## 不变量

- 未登录用户可以看界面，但发送、切换、保存、测试、审计查看和审计导出等受限操作必须打开登录浮窗或被拦截。
- 登录用户默认只读写自己的“我的历史”；当前角色下其他用户历史可以通过共享历史选择器只读查看，查看时必须禁用发送。
- 生成监视不能占用聊天发送状态。
...
- `PipelineStepProgress.mindFlow` 只能作为临时聊天消息 streaming 展示。第一句发言完成后，pre-speech 心理流必须折叠；整轮完成后，post-speech 心理流也不能进入本地缓存、后台历史或审计消息 ID。
- 当 Reply LLM 有多段输出时，中间栏先展示第一句，让后置心理流继续 streaming；整轮完成后再追加后续分段，表现为“说完后心理余波推动再次发言”。
...
- `rg -n "App Shell|Conversation History|generationMonitor|PipelineStepProgress|replyOutput.segments|createConversationHistoryKey|sharedConversationHistoryAccess|conversationAuditExport" docs/AI_NAMING_REGISTRY.md docs/SYSTEM_FLOW.md`
- `rg -n "liveTrace|activeStep|mindFlow|upsertMindFlowChatMessage|foldTransientMindFlowMessages|authUser|personaDossier|normalizeReplySegments|conversation-history|conversation-state|resetActiveDossierConversation|selectedAuditIds|exportConversationAuditEntries" src/App.tsx src/chat`
