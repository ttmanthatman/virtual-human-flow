...
App Shell 负责三栏工作台、前端交互状态、房间时间线、聊天入口、消息渠道选择、现场事件触发、当前活动查看、当前上下文标签、连续回复分条呈现、心理/活动流折叠展开、流程追踪、旧兼容管线显示开关、生成监视、心理探针开关、档案切换、当前角色重置入口、共享角色历史选择器、管理员审计查看/选择/导出和浏览器端权限提示。它不负责服务端持久化规则、真实 DeepSeek 代理实现或认知模块内部判断。
