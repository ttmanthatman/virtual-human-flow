...
Server Support 负责 Node/Vite/生产服务共享的后端能力：登录会话、liao 登录代理、共享档案合并、运行时 JSON 文件、用户私有消息历史、角色全局对话运行态、审计、管理员审计导出、管理员历史读取、当前角色运行态重置和站内更新执行。它不负责前端 UI 布局或认知模块内部判断。
...
- 输出：auth session、persona dossiers、conversation histories、conversation states、conversation audits、conversation audit export、审计删除级联清理结果、角色重置清理结果、update status/SSE。
...
- `rg -n "requireSession|requireAdminSession|readPersonaDossiers|conversation-history|conversation-state|reset-conversation|conversation-audits|exportConversationAudits|deleteConversationArtifactsForAudit|app-update" serverSupport.mjs server.mjs vite.config.ts`
